...
const auditLogChain = require('../models')("AuditLogChain");
...
exports.AuditLogBlockchain = class AuditLogBlockchain {
    ...
    async getGenesisBlock() {
        let blockInfo = await auditLogChain.find().sort({ $natural: 1}).limit(1);
    ...
    async checkChainValidity() {
        let promise = new Promise((resolve) => {
            ...
            let idx = 1;
            auditLogChain.find({}).sort({$natural: 1}).cursor().on('data', entry => {
